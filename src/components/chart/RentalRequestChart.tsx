...
import { IGetContractCancellationRateByOwner, IGetRentalRequestRatingByOwner } from '../../types/dashboard';
import { fetchContractCancellationRate, fetchRentalRequestRating } from '../../api/contract';
...
const RentalRequestChart = () => {
    const [requestData, setRequestData] = useState<IGetRentalRequestRatingByOwner[]>([]);
    const [contractData, setContractData] = useState<IGetContractCancellationRateByOwner[]>([]);
            ...
            <View style={{ paddingHorizontal: 5 }}>
                    ...
                    <View style={{ flexDirection: 'row', justifyContent: 'center', marginHorizontal: 5 }}>
                        {combinedData.map((item, index) => (
                            <TouchableOpacity
                                ...
                                <Text style={{ color: 'white', fontSize: 14 }}>{`Thg ${item.month}`}</Text>
                            </TouchableOpacity>
                        ))}
                    </View>
                </ScrollView>
                ...
                >
                    ...
                    >
                        <LineChart
                            ...
                            xAxisLabelTexts={combinedData.map(item => `Thg ${item.month}`)}
                            ...
                            yAxisLabelWidth={30}
                            adjustToWidth={true}
                            yAxisTextStyle={{ fontSize: 12, textAlign: 'center' }}
                            initialSpacing={20}
                ...
                <View style={{ flexDirection: 'row', justifyContent: 'center', marginVertical: 10, marginBottom: 20 }}>
                    <View style={{ flexDirection: 'row', alignItems: 'center', marginHorizontal: 10 }}>
                        <View style={{ width: 10, height: 10, backgroundColor: '#2a9d90', marginRight: 5 }} />
                        <Text>Phê duyệt</Text>
                    </View>
                    <View style={{ flexDirection: 'row', alignItems: 'center', marginHorizontal: 10 }}>
                        <View style={{ width: 10, height: 10, backgroundColor: '#e76e50', marginRight: 5 }} />
                        <Text>Từ chối</Text>
                    </View>
                    <View style={{ flexDirection: 'row', alignItems: 'center', marginHorizontal: 10 }}>
                        <View style={{ width: 10, height: 10, backgroundColor: '#2563eb', marginRight: 5 }} />
