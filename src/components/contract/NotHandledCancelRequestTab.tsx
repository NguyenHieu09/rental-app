...
const NotHandledCancelRequestTab: React.FC<{ contractId: string }> = ({
  ...
  useEffect(() => {
    const loadRequests = async () => {
      try {
        const cancelRequestData = await fetchNotHandledCancelContractRequest(
          contractId
        );
  ...
  const handleStatusUpdate = async (newStatus: ContractCancelRequestStatus) => {
      ...
      {cancelRequest ? (
        <View style={styles.cancelRequestContainer}>
          ...
          <View style={styles.buttonContainer}>
            {isMyCancelRequest && cancelRequest.status === "PENDING" && (
              <Button
                ...
                loading={
                  cancelCancellationLoading === CANCEL_CANCELLATION_LOADING
                }
              >
                Hủy yêu cầu
              </Button>
            )}
            {isMyCancelRequest && cancelRequest.status === "REJECTED" && (
              <>
                <Button
                  style={commonStyles.flex1}
                  type="danger"
                  variant="outlined"
                  onPress={() => handleStatusUpdate("UNILATERAL_CANCELLATION")}
                  loading={
                ...
                <Button
                  ...
                  type="primary"
                  variant="fill"
                  onPress={() => handleStatusUpdate("CONTINUE")}
                  loading={
                    cancelCancellationLoading === CONTINUE_CANCELLATION_LOADING
                  }
                >
                  Tiếp tục thuê
                </Button>
            ...
            {/* FIXME */}
            {cancelRequest.status === "PENDING" && !isMyCancelRequest && (
              <>
                <Button
                  style={commonStyles.flex1}
                  type="danger"
                  ...
                  loading={
                    cancelCancellationLoading === REJECT_CANCELLATION_LOADING
                  }
                >
                  Từ chối
                </Button>
                <Button
                  ...
                  loading={
                    cancelCancellationLoading === APPROVE_CANCELLATION_LOADING
                  }
                >
                  Đồng ý
                </Button>
                  ...
                  <>
                    {user?.userTypes.includes("renter") ? (
                      <Button
                        ...
                        variant="outlined"
                        onPress={() =>
                          handleExtensionStatusUpdate(request.id, "CANCELLED")
                        }
                      >
                        Hủy yêu cầu
                      </Button>
                      ...
                      <>
                        <Button
                          ...
                          variant="outlined"
                          onPress={() =>
                            handleExtensionStatusUpdate(request.id, "REJECTED")
                          }
                        >
                          Từ chối
                        </Button>
                        <Button
                          ...
                          variant="fill"
                          onPress={() =>
                            handleExtensionStatusUpdate(request.id, "APPROVED")
                          }
                        >
                          Chấp nhận
                        </Button>
