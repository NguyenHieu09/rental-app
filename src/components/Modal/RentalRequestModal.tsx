...
const RentalRequestModal: React.FC<RentalRequestModalProps> = ({ isVisible, onClose, property, ownerId, userId }) => {
    ...
    const [startDate, setStartDate] = useState<Date | null>(null);
    const [endDate, setEndDate] = useState<Date | null>(null);
    const [deposit, setDeposit] = useState(''); // Sử dụng kiểu number
    const [rentalPrice, setRentalPrice] = useState(''); // Sử dụng kiểu number
    ...
    const handleRentalRequest = async () => {
        if (!startDate || !endDate || parseFloat(deposit) <= 0 || parseFloat(rentalPrice) <= 0) {
