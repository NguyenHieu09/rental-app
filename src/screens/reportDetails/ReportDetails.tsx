...
import {
  acceptProposal,
  acceptProposalByRenter,
  cancelReport,
  fetchReportDetails,
  ownerProposeReport,
  rejectProposalByRenter,
} from "../../api/contract";
    ...
    <ScrollView style={styles.container}>
      ...
      <View style={styles.section}>
        ...
        <View style={styles.row}>
          ...
          <Text>
            {report.reportChild[0]?.resolvedAt
              ? format(report.reportChild[0]?.resolvedAt, "dd/MM/yyyy")
              : ""}
          </Text>
        </View>
          ...
          <View style={styles.buttonContainer}>
            {user?.userTypes.includes("owner") ? (
              <>
                ...
                <Button type="primary" variant="fill" onPress={handleAccept}>
                  Đồng ý với yêu cầu
                </Button>
              </>
            ) : (
              <View style={{ width: "100%" }}>
                <Button type="danger" variant="outlined" onPress={handleCancel}>
                  Hủy
                </Button>
              </View>
            )}
      ...
      {report.reportChild[1] && (
        <View style={styles.section}>
          ...
          <View style={styles.row}>
            <Text style={styles.label}>Ngày giải quyết:</Text>
            <Text>
              {report.reportChild[1]?.resolvedAt
                ? format(report.reportChild[0]?.resolvedAt, "dd/MM/yyyy")
                : ""}
            </Text>
          </View>
      ...
      <View style={styles.section}>
        ...
        {report.history.map((item, index) => (
          <Text key={index}>
            {formatDateTime(item.createdAt)}: {getReportStatusText(item.status)}
          </Text>
        ))}
      </View>
