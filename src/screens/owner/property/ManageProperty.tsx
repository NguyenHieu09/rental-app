...
import { deleteProperty, fetchPropertiesWithFilters, updatePropertyVisibility } from '../../../api/api';
...
const ManageProperty: React.FC = () => {
    ...
    const loadProperties = async (page: number) => {
        try {
            ...
            if (total !== undefined) {
                setProperties((prevProperties) => (page === 0 ? properties : [...prevProperties, ...properties]));
    ...
    const handleUpdateVisibility = async (status: string) => {
        console.log(`${status === 'ACTIVE' ? 'Show' : 'Hide'} selected properties:`, selectedProperties);

        try {
            const response = await updatePropertyVisibility(selectedProperties, status);
            Alert.alert('Thành công', `${status === 'ACTIVE' ? 'Hiện' : 'Ẩn'} bất động sản thành công.`);
            setSelectedProperties([]);
            loadProperties(0); // Reload data after successful update
        } catch (error: any) {
            console.error(`Error ${status === 'ACTIVE' ? 'showing' : 'hiding'} properties:`, error);
