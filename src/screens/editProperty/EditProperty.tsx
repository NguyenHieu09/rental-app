      ...
      try {
        ...
        setAcreage(
          data.rentalConditions
            .find((condition: ICondition) => condition.type === "Diện tích")
            ?.value.split(" ")[0] || ""
        );
        setPrice(data.price.toString());
        setInterior(
          data.rentalConditions.find(
            (condition: ICondition) => condition.type === "Nội thất"
          )?.value || ""
        );
        setBedroom(
          data.rentalConditions
            .find((condition: ICondition) => condition.type === "Phòng ngủ")
            ?.value.split(" ")[0] || ""
        );
        setBathroom(
          data.rentalConditions
            .find((condition: ICondition) => condition.type === "Phòng tắm")
            ?.value.split(" ")[0] || ""
        );
        setLandArea(
          data.rentalConditions
            .find(
              (condition: ICondition) =>
                condition.type === "Diện tích quyền sử dụng đất"
            )
            ?.value.split(" ")[0] || ""
        );
        ...
        setFloor(
          data.rentalConditions
            .find((condition: ICondition) => condition.type === "Số tầng")
            ?.value.split(" ")[0] || ""
        );
    ...
    <View style={commonStyles.container}>
      <ScrollView>
        ...
        <TouchableOpacity
          ...
          disabled={loading}
        >
          <Text style={[styles.submitButtonText, commonStyles.buttonText]}>
