...
const EditPropertyScreen: React.FC = () => {
    ...
    useEffect(() => {
        const loadPropertyDetails = async () => {
            try {
                ...
                setAcreage(data.rentalConditions.find((condition: ICondition) => condition.type === 'Diện tích')?.value.split(' ')[0] || '');
                setPrice(data.price.toString());
                setInterior(data.rentalConditions.find((condition: ICondition) => condition.type === 'Nội thất')?.value || '');
                setBedroom(data.rentalConditions.find((condition: ICondition) => condition.type === 'Phòng ngủ')?.value.split(' ')[0] || '');
                setBathroom(data.rentalConditions.find((condition: ICondition) => condition.type === 'Phòng tắm')?.value.split(' ')[0] || '');
                setLandArea(data.rentalConditions.find((condition: ICondition) => condition.type === 'Diện tích quyền sử dụng đất')?.value.split(' ')[0] || '');
                setType({ id: data.type.id, name: data.type.name });
                setFloor(data.rentalConditions.find((condition: ICondition) => condition.type === 'Số tầng')?.value.split(' ')[0] || '');
                ...
                setSelectedAttributes(data.attributes.map((attr: IAttribute) => attr.name));
    ...
    const handleUpdate = async () => {
        ...
        try {
            ...
            const response = await updateProperty(propertyDetails?.propertyId || '', formData);
            if (response.success) {
                Alert.alert('Thành công', 'Cập nhật thành công.');
                navigation.navigate('PropertyDetail', { slug: propertyDetails?.slug || '' });
            } else {
            ...
            <ScrollView>
                ...
                <View style={styles.radioContainer}>
                    ...
                    {attributes.map((attribute) => (
                        <View key={attribute.name} style={styles.checkboxContainer}>
                            <Checkbox
                                color="#007BFF"
                                status={selectedAttributes.includes(attribute.name) ? 'checked' : 'unchecked'}
                                onPress={() => handleAttributeChange(attribute.name)} // Thêm hàm xử lý khi thay đổi checkbox
                            />
